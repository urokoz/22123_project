...
mean_expression <- function(df, classes){
  for (class in unique(classes)) {
    ...
    # save the files
    write.table(mean_class,file = sprintf('data/top_or_buttom_25/pure/%s_class_GBM.txt', class))
    write.table(mean_rest,file = sprintf('data/top_or_buttom_25/pure/%s_rest_GBM.txt',class))
  }
}
...
Overlap_genes <- function(subtypes, percentage){
  temp_class <- read.table(sprintf('data/top_or_buttom_25/pure/%s_class_GBM.txt',subtypes))
  temp_rest  <- read.table(sprintf('data/top_or_buttom_25/pure/%s_rest_GBM.txt',subtypes))
  ...
  {write(overlap, file =sprintf('data/top_or_buttom_25/pure/0.35/%s_signatures_GBM.txt',subtypes))}}
